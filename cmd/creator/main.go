...
func main() {
	...
	flagx.Var(&icon, "icon", "", "icon file path")
	flagx.Var(&targetDir, "target-dir", "d", "target file path")
	flagx.Struct(&app)
	flagx.Parse()
	...
	if err := Create(&app, targetDir, icon); err != nil {
		slog.Error("create app", "err", err)
		os.Exit(1)
	}
	slog.Info("create app", "target", targetDir, "url", app.Url)
}

func Create(app *base.WebApp, targetDir, icon string) error {
	targetDir, err := filepath.Abs(targetDir)
	if err != nil {
		return err
	}

	appUri, err := url.Parse(app.Url)
	if err != nil {
		return err
	}

	if app.Name == "" {
		if targetDir == filepath.VolumeName(targetDir)+`\` {
			app.Name = fsx.CleanFileName(appUri.Host + ".exe")
		} else {
	...
	target := filepath.Join(targetDir, app.Name+".exe")
	...
	binary.LittleEndian.PutUint32(magic[4:], base.Magic)
