...
import (
	...
	"github.com/cnk3x/win-webapp-gen/webview2/internal/w32"
	"github.com/cnk3x/win-webapp-gen/webview2/pkg/edge"

	"golang.org/x/sys/windows"
)

var (
	windowContext     = map[uintptr]any{}
	windowContextSync sync.RWMutex
)

func getWindowContext(wnd uintptr) any {
	windowContextSync.RLock()
	defer windowContextSync.RUnlock()
	return windowContext[wnd]
}

func setWindowContext(wnd uintptr, data any) {
...
func NewWithOptions(options WebViewOptions) WebView {
	...
	err = settings.PutIsPasswordAutosaveEnabled(true)
	if err != nil {
		log.Fatal(err)
	}

	err = settings.PutIsGeneralAutofillEnabled(true)
	if err != nil {
		log.Fatal(err)
	}
...
func jsString(v any) string { b, _ := json.Marshal(v); return string(b) }
...
func (w *webview) callbinding(d rpcMessage) (any, error) {
	w.m.Lock()
